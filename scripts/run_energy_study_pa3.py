"""Campaign A energy study — PA_3 steps with naphtha+kero rate holds.
...
Usage (HYSYS T-100 open):
  .venv\\Scripts\\python.exe scripts/run_energy_study_pa3.py
"""
...
def judge(before: SnapshotMetrics, after: SnapshotMetrics) -> tuple[str, str]:
    ...
    if dnet < -NET_WIN_ABS:
        return "KEEP", f"net |PA3|+|Cond| down ({basis}: {dnet:.4g}); dPA={dpa}; dCond={dc}"
...
def main() -> int:
    ...
    print("=" * 72)
    print("ENERGY STUDY Campaign A — PA_3 with Naphtha+Kero holds")
    print("=" * 72)
    ...
    print(
        f"BASELINE state={baseline.state} PA3={baseline.pa_disp} "
    ...
    for i in range(1, MAX_STEPS + 1):
        ...
        print(f"\n[STEP {i}] PA_3_Duty Goal {p.goal_value} -> {new_goal} ({STEP_FRAC}x |duty|)")
        ...
        print(
            f"  after PA3={after.pa_disp} CondQ_disp={after.cond_q_disp} "
    ...
    payload = {
        "study": "T-100 Campaign A energy — PA_3",
        "when_utc": stamp,
    ...
    json_path = out_dir / f"energy_pa3_run_{stamp}.json"
    json_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")

    md_path = out_dir / f"energy_pa3_run_{stamp}.md"
    lines = [
        f"# Energy study run — PA_3 ({stamp} UTC)",
        "",
        "**Column:** T-100  ",
        "**Holds:** Naphtha Prod Rate, Kero_SS Prod Flow  ",
    ...
    lines.extend(
        [
            ...
            "- **Energy:** same-unit |PA3|+|Cond| (display preferred; COM fallback). "
            ...
            "Case restored to baseline PA_3. Campaign A PA weaker-cooling series complete after this run.",
            "",
        ]
    )
